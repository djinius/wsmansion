...
label part1:
    ...
    독백 "……."
    독백 "푹신한 소파에 앉아 주위를 둘러보았다."
    독백 "바깥부터 내부까지, 고풍스러운 걸 넘어 과거의 유물 같은 곳이다."
    독백 "가구는 물론 웬만한 골동품 가게에서도 보기 힘든 물건들로 빼곡하다."
    ...
    독백 "'기억을 잃어버렸다'는, 단순하다 못해 성의 없는 변명을 누가 믿어주겠는가."
    # 소녀? (보통, 웃음, 미소)
    아비게일처음 입미소 "음…무언가 말로는 못 할 사정이 있는 것 같은데, 맞으실까요?"
    독백 "놀랍게도 소녀는 내게 의심의 눈초리 대신 따스한 미소를 건네주었다."
    독백 "고개를 끄덕이자 소녀의 미소가 더욱 밝아졌다."
    # 소녀? (행복, 실눈 웃음, 큰 웃음)
    아비게일처음 눈썹행복 눈실눈웃음 입큰웃음 "후훗, 그래도 너무 걱정하지는 마세요." 
    아비게일처음 "근처의 숲에서 길을 잃고 조난당한 분들도 가끔 찾아오시거든요."
 
    # 소녀? (침울, 슬픔, 무표정)
    아비게일처음 눈썹침울 눈침울 입무표정 "오히려 지금 당장 도와드릴 수 없어 죄송합니다."
    ...
    독백 "…순간적으로 뇌리를 스치는 느낌에 나는 다시 자리를 고쳐 앉았다."
    ...
    독백 "또 {i}말 못할 상황{/i}인건 어떻게 알고 있었지?"
    ...
    독백 "소녀의 삐죽한 덧니가 빈 찻잔에 반사되어 반짝였다."
    stop music
    ...
    scene living
    # 소녀? (놀람, 실눈 웃음, 큰 웃음)
    show 아비게일처음 눈썹행복 눈실눈웃음1 입큰웃음:
        pos (.5, .05) anchor (.5, .0)
        zoom .5
    with dissolve

    독백 "순간적으로 놀라 움츠러든 내 앞에 소녀가 크게 입을 벌리고 있었다."
    독백 "양팔을 벌리고, 모든 손가락을 세우고서 말이다."
    # 소녀? (놀람, 놀람, 무표정)
    ...
    play music tranquil1
    독백 "멋쩍게 웃어 보이는 걸로 대답을 대신했다."
    독백 "살짝 당황하긴 했어도 내겐 썩 귀여운 장난이었다."
    ...
    show 아비게일 눈썹행복 눈웃음 입미소:
        pos (.5, .05) anchor (.5, .0)
        zoom .5
    아비게일 "처음 뵙겠습니다, 제 이름은 아비게일, {i}아비게일 F. 엘리엇.{/i}"
    ...
    독백 "자기소개도 끝마친 참이지만, 미심쩍은 느낌이 모두 가신 건 아니었다."
    독백 "갈피가 잡히지 않는 상황, 조각상, 오래된 저택, 그리고 엘리…."
    독백 "무언가 실마리가 보일 듯하면서도 연결되지 않는다. 기억의 빈자리가 크다."
    독백 "날 도와줄 누군가는 올 기미가 없고, 엘리를 100\% 신뢰하지 못하는 지금,\n오로지 내 힘으로 이 저택에 얽힌 비밀을 풀어 집으로 돌아가야만 한다."
    ...
    독백 "저택 이곳저곳을 지날 때마다 어렴풋한 광경이 떠오르는 것 같은 느낌을 받았다."
    독백 "그 장소들을 조사하다 보면 내 기억을 되찾으리라고 확신했다."
    독백 "다시 엘리가 떠난 사이, 이번엔 무언가, 무언가 단서를 찾아야만 한다."
...
label investigateTable:
    ...
    # cg: 두꺼운 책
    독백 "책상 위에 두꺼운 책이 놓여있다. 아가씨답게 고상한 취미다."
    ...
    # 엘리 (행복, 웃음, 미소)
    엘리 눈웃음 "아, 꽃 이름이에요. 달처럼 은은하게 빛난다고 해서 문빔이랍니다."
    엘리 "한 송이 따서 책갈피로 만들어 봤어요. 예쁘죠?"
...
label investigateRefrigerator:
    ...
    extend " 콜라병들이었다! 그것도 아주 많이."
    # object: coke
    show coke:
        align (.5, .5) zoom .5
    with dissolve
    독백 "이놈의 흡혈귀는 피가 아니라 콜라를 먹고 사나 싶을 정도로\n콜라병들이 냉장고 안을 빽빽이 채우고 있었다."
...
label investigateCupboard:
    ...
    독백 "옛날 생각에 엘리의 눈치를 보며\n껌 하나를 입에 넣고 부드러워질 때까지 질겅질겅 씹었다."
    독백 "그리고 풍선을 만들어 불려고 했는데… 생각보다 잘 되지 않는다."
    # 엘리 등장
    # 엘리 (보통, 웃음, 무표정)
